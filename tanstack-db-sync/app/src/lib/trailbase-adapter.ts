import { initClient, type Client, type RecordApiImpl } from "trailbase";

// Wrapper to add createBulk method to TrailBase RecordApi
export function wrapRecordApi(recordApi: RecordApiImpl): RecordApiImpl & { createBulk: <T>(records: T[]) => Promise<number[]> } {
  ...
  for (const key in recordApi) {
    if (Object.prototype.hasOwnProperty.call(recordApi, key)) {
      wrapper[key] = recordApi[key as keyof RecordApiImpl];
    }
  }
  ...
  return wrapper as RecordApiImpl & { createBulk: <T>(records: T[]) => Promise<number[]> };
}
...
export function createClient(url: string): Client {
  ...
  client.records = function(name: string) {
    ...
    return wrapRecordApi(recordApi as RecordApiImpl);
  };
